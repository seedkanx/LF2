...
static gi(t) {
        ...
        FouV.kt[t] = !0,
        // [修改] 移除了 window.electronAPI.V6rM(t)
        FouV.fi(),
        GqLZ ? (s = FouV.A(),
...
static ui() {
        ...
        2 == FouV.T)
            ...
            if (1 != FouV.Et) {
                ...
                !FouV.Xt) {
                    function s(t) {
                        // [修改 1] 移除 Node.js 環境專用的 Buffer，強制使用網頁標準解碼
                ...
                // [修改 2] 移除所有 window.electronAPI.eBgCtl()、NuvU() 與 gZN5() 的非同步依賴
                // 直接執行本地玩家的網頁版環境初始化
                0 == FouV.Et && (
                    FouV.Et = 1, // 直接跳轉至 1 進入 Loading 畫面
                    
                    (()=>{
                        let s = localStorage.getItem("user") || "WebPlayer", e = localStorage.getItem("user");
                        ...
                        function f(t) {
                            ...
                            localStorage.setItem(p(t), g(t))) // [修改 3] 移除了 Steam 成就補登 API
                        }
                        
                        // 保留：自動向下解鎖低難度成就的功能
                        for (let t = 1; t < 10; t++)
                            FouV.kt[h + t + u] && (f(h + t + l), f(h + t + r), f(h + t + n)),
                            FouV.kt[h + t + l] && (f(h + t + r), f(h + t + n)),
                            FouV.kt[h + t + r] && f(h + t + n);
                        
                        FouV.fi();
                        FouV.bt = s;
                        Sh7E.ki = !1; // 強制不使用 Steam Deck 特殊解析度模式
                    })()
                )
            } else if (1 == FouV.Et)
                if (FouV.Bt < 100) {
                ...
                D.C_wB('<a onclick="globalUpdateLog()" href="#" style="color: #c5d1ff; text-shadow: #000 1px 0 3px;">Update Log </a>', 30, h - 54, "OkQn", "#fff")),
                0 == FouV.page) {
                    let s = 0
                      , e = 0
                      , i = 0;
                    ...
                    0 < FouV.L)
                        ...
                        D.QZUl(),
                        FzMO(UI_STATE_NETWORK_MODE_MENU_PAGE))) : 
                        3 == FouV.qt ? (D.Dm9f(BLg_.xi[9], FouV.Kt + 14 * FouV.wt, FouV.$t + 76 * FouV.wt, 0, FouV.Dt),
                        FouV.F && (FouV.F = !1,
                        XYSz.D(0, 1),
                        FouV.page = 60,
                        // [修改 4] 移除了 GqLz && GLOBAL_STEAM_MESSAGE，保留 GqLZ 網頁版 WebSocket
                        GqLZ && _dFL.send(`AWAY\n${WEPA}\ncontrol_setting`))) : 
                        4 == FouV.qt ? (D.Dm9f(BLg_.Gi[1], FouV.Kt + 16 * FouV.wt, FouV.$t + 107 * FouV.wt, 0, FouV.Dt),
                        FouV.F && (FouV.F = !1,
                        XYSz.D(0, 1))) : // [修改 5] 移除了 window.electronAPI.b1hy()
                        5 == FouV.qt ? (D.Dm9f(BLg_.Gi[2], FouV.Kt + 16 * FouV.wt, FouV.$t + 138.7 * FouV.wt, 0, FouV.Dt),
                        FouV.F && (FouV.F = !1,
                        XYSz.D(0, 1))) : // [修改 6] 移除了 window.electronAPI.GF6E()
                        6 == FouV.qt && FouV.F && (FouV.F = !1,
                        XYSz.D(0, 1));   // [修改 7] 移除了 window.electronAPI.Tf5c()
